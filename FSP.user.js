...
(function () {
    ...
    function fitScheduleText(item) {
        if (!item) return;

        item.style.setProperty('background', '#fff0f5', 'important');
        item.style.setProperty('background-image', 'none', 'important');
        item.style.setProperty('border', '1px solid #efc3d3', 'important');

        item.querySelectorAll('.v-expand, .v-slot, .v-verticallayout').forEach((element) => {
            element.style.setProperty('background', '#fff0f5', 'important');
            element.style.setProperty('background-image', 'none', 'important');
        });

        const labels = item.querySelectorAll('.v-label');

        labels.forEach((label) => {
            label.style.setProperty('color', '#513b45', 'important');
    ...
    const css = `
        ...
        .v-app .v-nativebutton {
            ...
            font: 600 10px/1.2 "Nunito", "Segoe UI", Arial, sans-serif !important;
        ...
        .v-app .v-button:hover,
        .v-app .v-nativebutton:hover {
        ...
        .v-app .v-button.quiet:hover,
        .v-app .v-button-quiet:hover {
            background: #f8dce7 !important;
            border-color: #d486a4 !important;
        }

        .v-app .v-button:focus,
        .v-app .v-nativebutton:focus,
        .v-app a:focus,
        .v-app input:focus,
        .v-app textarea:focus,
        .v-app select:focus {
        ...
        .v-app .v-information-component-info,
        .v-app .v-information-component-warning {
        ...
        .v-app .schedule-item {
            ...
            border: 1px solid #efc3d3 !important;
            border-radius: 8px !important;

            background: #fff0f5 !important;
            background-image: none !important;

            box-shadow: 0 2px 5px rgba(126, 68, 94, 0.10) !important;
        }

        /* Внутренние контейнеры Vaadin */
        .v-app .schedule-item .v-expand,
        .v-app .schedule-item .v-slot,
        .v-app .schedule-item .v-verticallayout {
            background: #fff0f5 !important;
            background-image: none !important;
        }

        /* Текст пары.
           Не меняем display, width, padding и позиционирование,
           чтобы не ломать разметку Vaadin. */
        .v-app .schedule-item .v-label,
        .v-app .schedule-item .v-label * {
            background: transparent !important;
            background-image: none !important;

            color: #513b45 !important;
            -webkit-text-fill-color: #513b45 !important;
        ...
        /* При наведении */
        .v-app .schedule-item:hover {
            border-color: #d9a6ba !important;
            background: #fbe3ec !important;

            box-shadow: 0 4px 9px rgba(126, 68, 94, 0.14) !important;
        }

        .v-app .schedule-item:hover .v-expand,
        .v-app .schedule-item:hover .v-slot,
        .v-app .schedule-item:hover .v-verticallayout {
            background: #fbe3ec !important;
        }

        .v-app .v-information-component {
            border: 1px solid var(--pastel-border) !important;
            border-radius: 10px !important;
            background: #fff9fb !important;
            box-shadow: 0 3px 10px rgba(126, 68, 94, 0.08) !important;
            color: var(--pastel-text) !important;
        }

        .v-app .v-information-component td {
            border-color: #f0d9e2 !important;
            color: var(--pastel-text) !important;
        }

        .kbtu-pastel-menu-source {
        ...
        body.kbtu-drawer-open #kbtu-navigation-drawer {
        ...
        body.kbtu-drawer-open #kbtu-drawer-backdrop {
            display: block;
        }
    ...
    function installFont() {
        ...
        font.rel = 'stylesheet';
        font.href =
            'https://fonts.googleapis.com/css2?family=Comfortaa:wght@600;700&family=M+PLUS+Rounded+1c:wght@600&family=Nunito:wght@400;500;600;700&display=swap';
    ...
    function installFooter() {
        ...
        footer.innerHTML =
            '<a href="https://github.com/App1epli" target="_blank" rel="noopener noreferrer">github.com/App1epli</a>';
    ...
    function installNewsHeader() {
        const firstInfoNotice = document.querySelector(
            '.v-information-component-info'
        );

        if (
            !firstInfoNotice ||
            firstInfoNotice.querySelector('.kbtu-news-header-image')
        ) {
            return;
        }
    ...
    function improveScheduleItems() {
        const items = [...document.querySelectorAll('.schedule-item')];

        items.forEach(fitScheduleText);

        if (!scheduleResizeObserver && 'ResizeObserver' in window) {
            scheduleResizeObserver = new ResizeObserver(() => {
                document
                    .querySelectorAll('.schedule-item')
                    .forEach(fitScheduleText);
            });
        }

        if (scheduleResizeObserver) {
            items.forEach((item) => {
                if (!item.dataset.kbtuScheduleObserved) {
                    scheduleResizeObserver.observe(item);
                    item.dataset.kbtuScheduleObserved = 'true';
                }
            });
        }
    }
    ...
    function installNavigation() {
        ...
        const allLinks = [
            ...document.querySelectorAll('.v-link a[href]')
        ];

        const primaryPaths = new Set(
            primaryLinks.map(({ path }) => path)
        );

        const firstPrimaryLink = allLinks.find((link) =>
            primaryPaths.has(getPath(link))
        );
        ...
        const menuLinks = [
            ...sourcePanel.querySelectorAll('.v-link a[href]')
        ];

        const linksByPath = new Map(
            menuLinks.map((link) => [getPath(link), link])
        );
        ...
        primaryLinks.forEach(({ path }) => {
            ...
            if (source) {
                quickActions.appendChild(
                    copyLink(source, 'kbtu-quick-action')
                );
            }
        });
        ...
        drawer.innerHTML =
            '<button id="kbtu-drawer-close" type="button">×</button>' +
            '<nav id="kbtu-drawer-links"></nav>';

        const drawerLinks = drawer.querySelector(
            '#kbtu-drawer-links'
        );

        menuLinks.forEach((link) => {
            if (!primaryPaths.has(getPath(link))) {
                drawerLinks.appendChild(
                    copyLink(link, 'kbtu-drawer-link')
                );
            }
        });
        ...
        drawer
            .querySelector('#kbtu-drawer-close')
        ...
        sourcePanel.parentElement.insertBefore(
            quickActions,
            sourcePanel
        );
    ...
    function watchForNavigation() {
        if (
            !document.body ||
            document.documentElement.dataset.kbtuPastelWatching
        ) {
            return;
        }
